...
class FarmingInterfaceLeft(QWidget):
    ...
    def _disable_setting(self, parent):
        for child in parent.children():
            ...
            if not isinstance(child, QWidget):
                continue
            if child.objectName() in ("link_start", "pause_resume"):
                continue
            # 检查是否为目标控件类型
            if isinstance(
                child,
    ...
    def handle_script_finished(self):
        ...
        if script is not None and script.exception is None and not self._stop_in_progress:
            # 正常跑完时底层已做窗口/模拟器收尾，这里只恢复 UI，避免重复副作用。
