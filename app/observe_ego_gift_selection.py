...
优先使用新格式 ``system_level_row_col``，同时兼容旧版 ``system_gift_level_index.png``。
"""
...
_LEGACY_FORMAT_PATTERN = re.compile(r"^([a-z]+)_gift_(\d+)_(\d+)\.png$")
...
def parse_observe_ego_gift_value(value: str) -> ObserveGiftSelection | None:
    ...
    legacy_match = _LEGACY_FORMAT_PATTERN.match(value)
    if legacy_match is None:
        return None

    index = int(legacy_match.group(3))
    if index <= 0:
        return None

    selection = ObserveGiftSelection(
        system=legacy_match.group(1),
        level=int(legacy_match.group(2)),
        row=(index - 1) // 8 + 1,
        col=(index - 1) % 8 + 1,
    )
    return selection if selection.is_complete() else None
