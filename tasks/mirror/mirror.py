...
class Mirror:
    ...
    def enter_mir_with_star(self):
        ...
        while True:
            ...
            if auto.find_element("mirror/road_to_mir/dreaming_star/select_star_confirm_assets.png"):
                if bonuses_selected:
                    auto.click_element("mirror/road_to_mir/dreaming_star/select_star_confirm_assets.png")
                    continue
                if auto.click_element(
            ...
            if not bonuses_selected:
                ...
                if level_count == bonus_num:
                    ...
                    selected_indices = [index for index, bonus_value in enumerate(self.opening_bonus) if bonus_value >= 1]
                    if isinstance(self.opening_bonus_order, list):
                        ordered_pairs = []
                        for index in selected_indices:
                            if index < len(self.opening_bonus_order):
                                try:
                                    order = int(self.opening_bonus_order[index])
                                except (TypeError, ValueError):
                                    order = 0
                                if order > 0:
                                    ordered_pairs.append((order, index))
                        ordered_indices = [index for _, index in sorted(ordered_pairs)]
                        selected_indices = ordered_indices + [
                            index for index in selected_indices if index not in ordered_indices
                        ]

                    for index in selected_indices:
                        auto.mouse_action_with_pos((starlights_X[index], starlights_Y[index]))
                    ...
                    for index in selected_indices:
                        if self.opening_bonus[index] == 2:
                            auto.mouse_action_with_pos(
                                (
                                    starlights_X[index] - 80 * scale,
                                    starlights_Y[index] + 320 * scale,
                                )
                            )
                            sleep(cfg.mouse_action_interval)
                        elif self.opening_bonus[index] == 3:
                            auto.mouse_action_with_pos(
                                (
                                    starlights_X[index] + 80 * scale,
                                    starlights_Y[index] + 320 * scale,
    ...
    def select_init_ego_gift(self):
        ...
        while True:
            ...
            if auto.find_element("mirror/theme_pack/feature_theme_pack_assets.png") or auto.find_element(
                "mirror/road_to_mir/observe_ego_gift/observe_bleed_assets.png"
    ...
    def select_observe_ego_gift(self):
        ...
        def _select_gift(level_point):
            first_gift = (level_point[0], level_point[1] + 80 * my_scale)
            select_gift_point = (
                first_gift[0] + (gift_col - 1) * 165 * my_scale,
                first_gift[1] + (gift_row - 1) * 160 * my_scale,
            )
            if select_gift_point[1] < gift_box[-1]:
                auto.mouse_action_with_pos(select_gift_point)
                sleep(cfg.mouse_action_interval)
            else:
                step = 300 * my_scale
                height = step
                ...
                for s in result:
                    ...
                    sleep(1)
                auto.mouse_action_with_pos((select_gift_point[0], select_gift_point[1] - height))
                sleep(cfg.mouse_action_interval)
        ...
        for gift_id in self.observe_ego_gift_selected:
            legacy_match = re.match(r"^([a-z]+)_gift_(\d+)_(\d+)\.png$", gift_id)
            if legacy_match:
                file_system = legacy_match.group(1)
                gift_level = int(legacy_match.group(2))
                legacy_index = int(legacy_match.group(3))
                if legacy_index <= 0:
                    log.warning(f"旧版观测饰品索引无效：{gift_id}，跳过")
                    continue
                gift_row = (legacy_index - 1) // 8 + 1
                gift_col = (legacy_index - 1) % 8 + 1
            else:
                match = re.match(r"^([a-z]+)_(\d+)_(\d+)_(\d+)$", gift_id)
                if not match:
                    log.warning(f"无法解析饰品体系：{gift_id}，跳过")
                    continue
                file_system = match.group(1)
                gift_level = int(match.group(2))
                gift_row = int(match.group(3))
                gift_col = int(match.group(4))
            ...
            if reset_indices:
                ...
                sleep(cfg.mouse_action_interval)
            auto.mouse_action_with_pos((benchmark_point[0] + 110 * system_index * my_scale, benchmark_point[1]))
            sleep(cfg.mouse_action_interval)
        ...
        for _ in range(5):
            ...
            if ocr_result:
                auto.mouse_action_with_pos(((bbox[0] + bbox[2]) / 2, (bbox[1] + bbox[3]) / 2))
                sleep(cfg.mouse_action_interval)
                sleep(1)
        ...
        for _ in range(3):
            bbox = ImageUtils.get_bbox(ImageUtils.load_image("mirror/road_to_mir/observe_ego_gift/reject_gift_bbox.png"))
        ...
        while True:
            ...
            try:
                ...
                if len(acquire_card) == 2:
                    ...
                    seen_x = []
                    unique_buttons = []
                    for button in sorted(acquire_card, key=lambda p: p[0]):
                        if not any(abs(button[0] - sx) < 50 for sx in seen_x):
                            seen_x.append(button[0])
                            unique_buttons.append(button)
                    acquire_card = unique_buttons
                    system_nums = 0
                    for button in acquire_card:
                        bbox = (
                            button[0] - 50 * my_scale,
