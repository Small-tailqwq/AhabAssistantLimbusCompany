...
class Automation(metaclass=SingletonMeta):
    ...
    def _load_template_for_path(self, target: str, target_path: str, cacheable: bool, resize: bool = True):
        cache_key = (target, target_path, "resize" if resize else "noresize")
        if cacheable and cache_key in self.img_cache:
            cached = self.img_cache[cache_key]
            return cached["template"], cached["bbox"]

        template = ImageUtils.load_from_specific_path(target, target_path, resize=resize)
        if template is None:
            return None, None
        if target.endswith("assets.png"):
            bbox = ImageUtils.get_bbox(template)
